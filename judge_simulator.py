...
# Optional environment overrides (keeps file-edit workflow intact while enabling
# automation scripts and CI-style runs).
BOT_URL = os.getenv("JUDGE_BOT_URL", BOT_URL)
LLM_PROVIDER = os.getenv("JUDGE_LLM_PROVIDER", LLM_PROVIDER)
LLM_API_KEY = os.getenv("JUDGE_LLM_API_KEY", LLM_API_KEY)
LLM_MODEL = os.getenv("JUDGE_LLM_MODEL", LLM_MODEL)
OLLAMA_URL = os.getenv("JUDGE_OLLAMA_URL", OLLAMA_URL)
TEST_SCENARIO = os.getenv("JUDGE_TEST_SCENARIO", TEST_SCENARIO)
...
class MockProvider(LLMProvider):
    """Offline scoring provider for local harness runs."""

    def __init__(self, model: str = ""):
        self.model = model or "mock-scorer-v1"
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        ...
        if "Say 'ready' if you can hear me." in prompt:
            return "ready"

        # Scoring flow expects strict JSON output.
        if "SCORE THIS MESSAGE" in prompt:
            body_match = re.search(r'Body \((\d+) chars\): "([\s\S]*?)"\nCTA:', prompt)
            message_body = body_match.group(2) if body_match else ""
            nums = len(re.findall(r"\d+", message_body))
            lower = message_body.lower()
            specificity = min(10, 4 + nums * 2)
            category_fit = 8 if any(k in lower for k in ["dr.", "clinic", "offer", "review", "ctr"]) else 6
            merchant_fit = 8 if any(k in lower for k in ["your", "ctr", "offer", "local"]) else 6
            decision_quality = 8 if any(k in lower for k in ["want me", "reply", "next step"]) else 6
            engagement = 8 if any(k in lower for k in ["want me", "reply", "ready", "plan"]) else 6
            return json.dumps(
                {
                    "specificity": specificity,
                    "specificity_reason": "Contains measurable anchors and concrete details.",
                    "category_fit": category_fit,
                    "category_fit_reason": "Tone broadly matches merchant-assistant style.",
                    "merchant_fit": merchant_fit,
                    "merchant_fit_reason": "Uses merchant-specific framing where available.",
                    "decision_quality": decision_quality,
                    "decision_quality_reason": "Message links to trigger and includes a clear next action.",
                    "engagement_compulsion": engagement,
                    "engagement_reason": "Includes low-friction follow-up prompts.",
                    "hint": "Tighten specificity and persona matching for higher scores.",
                }
            )
